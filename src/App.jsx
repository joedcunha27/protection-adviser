...
RULES:
LIFE INSURANCE: If mortgage exists, primary goal = pay it off if either person dies. After mortgage cleared, calculate remaining outgoings (total outgoings minus mortgage payment). Check if surviving partner income covers remaining outgoings with at least £500/month to spare. If their income minus remaining outgoings is less than £500, there is a shortfall — recommend FIB for that shortfall amount per person separately. No mortgage and renting = use total outgoings including rent as the base. Single with no dependants = NO standalone life insurance. CIC and IP only.

INCOME PROTECTION: Maximum insurable amount = 60% of gross salary (pays out tax free, so do not use net take-home as the basis). For the recommended cover amount, use the following logic: (a) If two people and their gross incomes are within 25% of each other = recommend 50% of total household outgoings each, so that each person's IP covers their share and if both are off sick the full outgoings are covered. (b) If one person earns significantly more than the other = weight the IP amounts proportionally to their income split. (c) Single person = 60% of gross. In all cases the recommended amount must not exceed 60% of gross. Deferred period: (1) SE/no sick pay AND manual/trade = 1 month. (2) SE/no sick pay AND white collar/office AND savings >= 3 months outgoings = 3 months. (3) SE/no sick pay AND white collar AND low savings = 1 month. (4) Employed with sick pay = deferral matches when sick pay ends. Always full-term own-occupation IP to state pension age (shown below). NEVER 2-year payment IP.

CRITICAL ILLNESS: Default 12 months net income per person, level term. Always recommend.

FAMILY INCOME BENEFIT: Where dependent children exist. Term = years until youngest reaches 21. Calculation: Start with total household outgoings, then SUBTRACT the mortgage payment (because the mortgage will already be paid off by the life cover). This gives remaining outgoings. Then check if the surviving partner's income covers those remaining outgoings with at least £500/month to spare. If their income minus remaining outgoings is less than £500, there is a shortfall. The FIB amount = remaining outgoings + £500 buffer minus surviving partner income. Apply per person separately — if both people have a shortfall they may have different FIB amounts. If no mortgage, use total outgoings including rent as the base.
